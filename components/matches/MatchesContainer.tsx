...
const matchesSearchTerm = (match: Match, searchTerm: string): boolean => {
  ...
  // Study preferences
  if (
    match.studyPreferences.preferredLocations.some((loc) =>
  ...
  if (
    match.studyPreferences.weeklyHours.toString().includes(searchLower) ||
    `${match.studyPreferences.weeklyHours}h`.includes(searchLower) ||
    `${match.studyPreferences.weeklyHours} hours`.includes(searchLower)
  )
    return true;

  // Noise level preferences
  const noiseLevel =
    match.studyPreferences.noiseLevel === 1
      ? "quiet"
      : match.studyPreferences.noiseLevel === 2
        ? "moderate"
        : "social";
...
export function MatchesContainer({
  ...
  const [deletedMatchIds, setDeletedMatchIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleStatusChange = (
    matchId: string,
    newStatus: "none" | "accepted" | "rejected"
  ) => {
